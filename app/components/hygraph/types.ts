...
//
/**
 * List of acceptable file extensions for Cloudinary Media
 *
 *  as outlined in the Cloudinary Cookbook (05/10/2023), see https://cloudinary.com/cookbook/converting_an_image_format
 */

export interface ResponsiveImageProps {
  id: string;
  mobileId?: Maybe<string>;
  variants: srcsetVariant[];
}

export type srcsetVariant = {
  trans: string[];
  windowWidth: number;
  imageWidth: number;
};
export type CloudinaryImage = {
  ...
  format: ImageFormatType;
  height: number;
  derived: Array<{url: string; secure_url: string; raw_transformation: string}>;
  version: number;
  duration: any;
  metadata: Array<any>;
  folder_id: string;
...
export type HygraphGlobalConfig = {
  password: string;
  closeWebsite: boolean;
  siteGoesLiveTime: string
